...
/**
 * Defines a triangle based mesh. A mesh is constructed by interpretting the
 * data given in an OFF file.
 * 
...
public class Mesh {
	...
	private Vector3[] computeNormals() {
		...
		// Compute per face normals and set the vertex normal to the average
		// normals across faces
		// to the vertex.
		try {
			for (int i = 0; i < 3 * getNumFaces(); i += 3) {
				
				/* compléter */
				Vector3 A = new Vector3(vertices[faces[i]]);
				Vector3 B = new Vector3(vertices[faces[i+1]]);
				Vector3 C = new Vector3(vertices[faces[i+2]]);
	...
	/**
	 * Returns the faces of the mesh. The returned array contains 3*n integers,
	 * with n the number of faces. Each integer is an index into the array of
	 * Vector.
	 */
	public int[] getFaces() {
		return faces;
